/**
 * LoopNode — React Flow custom node for loop modules.
 *
 * Similar to CompositeNode but with loop configuration (count/foreach/until).
 * Blue-purple color scheme, ⟳ icon.
 */

import React, { memo } from 'react';
import { Handle, Position, type NodeProps } from '@xyflow/react';
import type { PortDef } from './ModuleNode.js';

export interface LoopNodeData extends Record<string, unknown> {
  label: string;
  inputs: PortDef[];
  outputs: PortDef[];
  internalNodes: import('@xyflow/react').Node[];
  internalEdges: import('@xyflow/react').Edge[];
  loopConfig: {
...
function getLoopLabel(config: LoopNodeData['loopConfig']): string {
  switch (config.mode) {
    case 'count': return `×${config.count ?? 1}`;
    case 'foreach': return `遍历 ${config.foreachVar || ''}`;
    case 'until': return `直到 "${config.untilCondition || ''}"`;
  }
}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span>{getLoopLabel(nodeData.loopConfig)}</span>
        <span style={{ color: '#999' }}>{stepCount} 步</span>
      </div>
