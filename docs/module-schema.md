# dtool Studio 模块 Schema 草案 v0.1
...
- **模板优先**：Schema 的核心消费者是"模块导入系统"，不是执行引擎
...
flow:
  ...
  steps:
    - id: "步骤唯一ID"             # 供内部引用用
      ...
      config:                      # 模块参数（键值对）
        param1: "值"
        param2: "$param.参数ID"    # 引用模块参数：$param.参数ID
...
| 任意 → 参数 | 任何作用域引用模块参数：`$param.xxx` | ✅ 允许 |
...
以下规则在执行引擎层强制实施，模块定义时不需要显式标注，但作者应了解：
...
旧 dtool 的 DSL 格式（`input() name=raw` 行式语法）是为"人写"优化的。模块 Schema 是为"AI 生成 + 机器解析"优化的。
两者不冲突：模块 Schema 是存储格式，如果高级用户想手写，可以基于模块 Schema 做 DSL → YAML 转换器（未来功能）。
...
- 比 `{{param}}` 更精确：`$param` 明确作用域（模块参数），`$steps` 明确引用目标
